...
// ── Tiny inline icon glyphs used only inside this module (no extra deps) ────
const MiniIcon = {
  whatsapp: "M3 21l1.65-3.8a9 9 0 1 1 3.4 3.1z",
  ...
  book:     "M4 4h11a3 3 0 0 1 3 3v13H7a3 3 0 0 1-3-3z M4 4v13",
};
const Glyph = ({ d, size = 11, color }) => (
...
const Avatar = ({ name, color }) => {
  ...
  return (
    <div style={{ width: 34, height: 34, borderRadius: 9, background: color + '18', color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0, border: `0.5px solid ${color}33` }}>
      {initials || '?'}
    </div>
  );
};
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={S.pageEyebrow}>Students</div>
        ...
        <button onClick={() => setFormStudent(null)} style={{ padding: '10px 18px', borderRadius: 9, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: T.accent, border: 'none', color: '#fff', display: 'flex', alignItems: 'center', gap: 7, whiteSpace: 'nowrap', boxShadow: '0 2px 8px rgba(194,65,12,0.25)' }}>
          <Icon d={Icons.plus} size={13} />Add student
        </button>
      </div>

      {/* FILTER CONTROLS BAR */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center', marginTop: 16 }}>
        <div style={{ position: 'relative', flex: '1 1 220px', minWidth: 200, maxWidth: 320 }}>
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search student, parent, contact..." style={{ width: '100%', padding: '8px 10px 8px 30px', borderRadius: 8, border: `0.5px solid ${T.line}`, fontSize: 12, color: '#334155', outline: 'none', background: '#fff' }} />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ padding: '8px 10px', borderRadius: 8, border: `0.5px solid ${T.line}`, fontSize: 12, color: '#475569', background: '#fff', cursor: 'pointer' }}>
          {['All', 'Active', 'Paused', 'Completed', 'Dropped'].map(s => <option key={s} value={s}>{s === 'All' ? 'All statuses' : s}</option>)}
        </select>
        <select value={feeFilter} onChange={e => setFeeFilter(e.target.value)} style={{ padding: '8px 10px', borderRadius: 8, border: `0.5px solid ${T.line}`, fontSize: 12, color: '#475569', background: '#fff', cursor: 'pointer' }}>
          {['All', 'Paid', 'Pending', 'Overdue'].map(s => <option key={s} value={s}>{s === 'All' ? 'Any fee status (this month)' : `Fee: ${s}`}</option>)}
        </select>
        <span style={{ fontSize: 11, color: '#94a3b8', marginLeft: 'auto', fontWeight: 500 }}>{filtered.length} of {(students || []).length} shown</span>
      </div>

      {/* STUDENTS MAIN LEDGER TABLE — reorganized into clean, single-purpose columns */}
      <div style={S.tableCard}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 920 }}>
            <thead>
              <tr style={{ background: '#fafbfc' }}>
                {['Student', 'Age / Gender', 'Parent', 'Contact', 'Course', 'Tutor', 'Enrolled', 'Status', 'Fee (this month)', 'Actions'].map((h, i) => (
                  <th key={i} style={{ padding: '11px 14px', fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#94a3b8', textAlign: i === 9 ? 'right' : 'left', borderBottom: `0.5px solid ${T.line}`, whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={10} style={{ padding: 36, textAlign: 'center', fontSize: 13, color: '#94a3b8' }}>No students match these filters.</td></tr>
              ) : filtered.map(s => (
                <tr key={s._id} style={{ borderBottom: `0.5px solid #f1f5f9`, transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background = '#fafbfc'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>

                  {/* Student name + avatar only */}
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                      <Avatar name={s.studentName} color={T.teal} />
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{s.studentName}</div>
                    </div>
                  </td>

                  {/* Age / gender / country — separated out */}
                  <td style={{ padding: '12px 14px', fontSize: 12, color: '#475569' }}>
                    <div>{s.age} yrs · {s.gender}</div>
                    {s.country && <div style={{ fontSize: 11, color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}><Glyph d={MiniIcon.globe} color="#94a3b8" />{s.country}</div>}
                  </td>

                  {/* Parent only */}
                  <td style={{ padding: '12px 14px', fontSize: 13, fontWeight: 600, color: '#334155' }}>{s.parentName}</td>

                  {/* Contact: whatsapp + email, clearly stacked */}
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ fontSize: 12, color: '#16a34a', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}><Glyph d={MiniIcon.whatsapp} color="#16a34a" />{s.whatsapp}</div>
                    {s.email && <div style={{ fontSize: 11, color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 5, marginTop: 3 }}><Glyph d={MiniIcon.mail} color="#94a3b8" />{s.email}</div>}
                  </td>

                  {/* Course only */}
                  <td style={{ padding: '12px 14px', fontSize: 12, color: '#334155', fontWeight: 500 }}>{s.course || '—'}</td>

                  {/* Tutor only */}
                  <td style={{ padding: '12px 14px', fontSize: 12, color: '#64748b' }}>{s.tutorAssigned || '—'}</td>

                  {/* Enrollment date + source */}
                  <td style={{ padding: '12px 14px', fontSize: 12, color: '#64748b' }}>
                    <div>{s.enrollmentDate ? new Date(s.enrollmentDate).toLocaleDateString() : '—'}</div>
                    <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 2 }}>{s.source}</div>
                  </td>

                  <td style={{ padding: '12px 14px' }}><EnrollStatusBadge status={s.status} /></td>

                  <td style={{ padding: '12px 14px' }}>
                    <button onClick={() => setLedgerStudent(s)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <td style={{ padding: '12px 14px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <ActionBtn icon="wallet" color={T.teal} hoverBg={T.tealBg} hoverBorder={T.tealBd} title="Fee ledger history" onClick={() => setLedgerStudent(s)} />
...
const StudentFormWrapper = ({ student, courses = [], onClose, onSave, fieldStyle, labelStyle, inputStyle, grid2Style, T, Icon, Icons }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,24,48,0.5)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400, padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, width: 560, maxWidth: '100%', maxHeight: '90vh', overflowY: 'auto', padding: 26, boxShadow: '0 24px 64px rgba(10,24,48,0.3)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#0f172a' }}>{isEdit ? 'Modify student record' : 'Add new student'}</div>
          <button onClick={onClose} style={{ background: '#f1f5f9', border: 'none', cursor: 'pointer', width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}><Icon d={Icons.x} size={15} /></button>
        </div>

        <p style={{ fontSize: 11, fontWeight: 700, color: T.accent, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '10px 0 12px' }}>Core identity</p>
        ...
        <p style={{ fontSize: 11, fontWeight: 700, color: T.accent, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '14px 0 12px' }}>Guardian contact</p>
        <div style={grid2Style}>
          ...
          <div style={fieldStyle}><label style={labelStyle}>WhatsApp number *</label><input style={inputStyle} value={form.whatsapp} onChange={e => update('whatsapp', e.target.value)} /></div>
        ...
        <p style={{ fontSize: 11, fontWeight: 700, color: T.accent, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '14px 0 12px' }}>Academic & billing</p>
        <div style={grid2Style}>
          ...
          <div style={fieldStyle}><label style={labelStyle}>Monthly fee *</label><input style={inputStyle} type="number" value={form.monthlyFee} onChange={e => update('monthlyFee', e.target.value)} /></div>
          <div style={fieldStyle}><label style={labelStyle}>Currency</label><select style={inputStyle} value={form.currency} onChange={e => update('currency', e.target.value)}>{['USD','PKR','GBP','EUR','AED','CAD'].map(c => <option key={c}>{c}</option>)}</select></div>
        </div>

        <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px 0', borderRadius: 9, fontSize: 13, background: '#fff', border: `0.5px solid ${T.line}`, color: '#64748b', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
          <button onClick={executeSubmit} disabled={saving} style={{ flex: 1, padding: '11px 0', borderRadius: 9, fontSize: 13, fontWeight: 700, background: T.accent, border: 'none', color: '#fff', opacity: saving ? 0.7 : 1, cursor: 'pointer' }}>{saving ? 'Saving...' : isEdit ? 'Save changes' : 'Confirm enrollment'}</button>
        </div>
      </div>
    </div>
  );
...
const FeeLedgerWrapper = ({ student, onClose, onAddFee, onUpdateFee, onDeleteFee, T, Icon, Icons }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,24,48,0.5)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400, padding: 16 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, width: 500, maxWidth: '100%', padding: 24, boxShadow: '0 24px 64px rgba(10,24,48,0.3)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700 }}>Tuition billing ledger</div>
            <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>{student?.studentName} · {student?.currency} {student?.monthlyFee}/mo</div>
          </div>
          <button onClick={onClose} style={{ background: '#f1f5f9', border: 'none', cursor: 'pointer', width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}><Icon d={Icons.x} size={15} /></button>
        </div>

        {/* QUICK ADD NEW BILLING PERIOD */}
        <div style={{ display: 'flex', gap: 8, margin: '16px 0', padding: 12, background: '#fafbfc', borderRadius: 12, border: `0.5px solid ${T.line}`, flexWrap: 'wrap', alignItems: 'end' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>Month</span>
            <select style={{ padding: '7px 8px', borderRadius: 7, border: '0.5px solid #cbd5e1', fontSize: 12 }} value={fMonth} onChange={e => setFMonth(e.target.value)}>{MONTHS.map(m => <option key={m}>{m}</option>)}</select>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>Year</span>
            <input type="number" style={{ padding: '6px 8px', borderRadius: 7, border: '0.5px solid #cbd5e1', fontSize: 12, width: 66 }} value={fYear} onChange={e => setFYear(e.target.value)} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase' }}>Amount</span>
            <input type="number" style={{ padding: '6px 8px', borderRadius: 7, border: '0.5px solid #cbd5e1', fontSize: 12, width: 80 }} value={fAmount} onChange={e => setFAmount(e.target.value)} />
          </div>
          <button onClick={commitLedgerRow} disabled={loadingLedger} style={{ background: T.accent, color: '#fff', border: 'none', fontSize: 12, fontWeight: 700, padding: '8px 14px', borderRadius: 7, cursor: 'pointer', marginLeft: 'auto' }}>Generate invoice</button>
        </div>

        {/* RECORDED ITERATIONS LOOP */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: '40vh', overflowY: 'auto' }}>
          {sortedHistory.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 24, color: '#94a3b8', fontSize: 12 }}>No invoices generated yet.</div>
          ) : sortedHistory.map(row => row && (
            <div key={row._id} style={{ display: 'flex', alignItems: 'center', border: `0.5px solid ${T.line}`, borderRadius: 9, padding: '9px 12px' }}>
              ...
              {row.paidOn && <span style={{ fontSize: 10, color: T.sub, marginLeft: 8 }}>Paid {new Date(row.paidOn).toLocaleDateString()}</span>}
